...
the satellite's independent ACT1B accelerometer. The main point of the paper is that
reproducing the orbit accurately is not enough to trust the force you get out, so the
force has to be checked directly rather than inferred from a good trajectory fit.

## What's in here

Each notebook corresponds to a part of the paper:

- `gracefo_inverse_pinn.ipynb` (§4, §5.1–5.2) — the main iPINN estimator. Recovers the
...
- `synthetic_validation.ipynb` (§5.5) — the known-answer test where the true radial
...
arrays back into the same folder, so expect it to fill up as you run things.
...
The notebooks aren't fully independent — the comparison and propagation notebooks read
...
Concretely, the iPINN notebook writes `v7_full_Ft.npy` (and the matching `_Fr`, `_Fn`,
...
automatically, so later runs skip the slow astropy step.
